...
function handleFileUpload(event: Event) {
    ...
    if (target.files && target.files.length > 0) {
        ...
        reader.onload = (e) => { newProduct.imageUrl = e.target?.result as string; };
        reader.readAsDataURL(file);
    }
}

const isFormValid = () => {
    const requiredFields = [newProduct.name, newProduct.description, newProduct.category, newProduct.imageUrl];
    const textFieldsInvalid = requiredFields.some(field => !field || (typeof field === 'string' && field.trim() === ''));
    const requiredNumbers = [newProduct.price, newProduct.quantity];
    const numberFieldsInvalid = requiredNumbers.some(num => !num || num <= 0);
    return !textFieldsInvalid && !numberFieldsInvalid;
};

function createProduct() {
    if (!isFormValid()) {
        alert('Você deve preencher todos os campos para criar um anúncio!');
        return;
    }
    productStore.addProduct({ ...newProduct, id: Date.now() }); 
